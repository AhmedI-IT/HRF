...
<body>
    <div>
        ...
        <table dir="rtl" class="tab" width=100% border=1px solid black>
            ...
            // $quer = "SELECT *, MAX(date) FROM `bonus` GROUP BY e_id";
            // $quer = "SELECT *  FROM `bonus` where date <= NOW()  GROUP BY e_id DESC";
            // $quer = "SELECT employee.*,bonus.* FROM `bonus`,employee WHERE employee.id = bonus.e_id and bonus.date <= NOW()  GROUP BY bonus.e_id ";
            // $quer = "select bonus.* from bonus, (select e_id,max(date) as transaction_date from bonus group by e_id) max_user where bonus.e_id=max_user.e_id and bonus.date=max_user.transaction_date";
            $quer = "select employee.*,bonus.* from employee,bonus, (select e_id,max(date) as transaction_date from bonus group by e_id) max_user where employee.id = bonus.e_id and bonus.e_id=max_user.e_id and bonus.date=max_user.transaction_date";
            ...
            while ($row = mysqli_fetch_array($res)) {
                // $query = "SELECT bonus.*, scientific_title.*, direct.*, certificates.*, employee.* FROM bonus, scientific_title, direct, certificates, employee WHERE  scientific_title.e_id = bonus.e_id and direct.e_id = bonus.e_id and certificates.e_id = bonus.e_id and employee.id = bonus.e_id and bonus.date <= NOW() ORDER BY bonus.date DESC LIMIT 1";
