...
if (isset($_POST['submit'])) {
    ...
    $name = $_POST['name'];
    $query = "SELECT employee.*, dispatch.* FROM employee,dispatch WHERE employee.id = dispatch.e_id and dispatch.c_id = '" . $_SESSION['c_id'] . "' and employee.name like '%" . $name . "%'";
    $search_res = filterTable($query);
} else {
    $query = "SELECT employee.*, dispatch.* FROM employee,dispatch WHERE employee.id = dispatch.e_id and dispatch.c_id = '" . $_SESSION['c_id'] . "' ";
    $search_res = filterTable($query);
}
    ...
    <div class="searchs">
        ...
        <table class="tab" width=100% border=1px solid black>
            ...
            while ($row = mysqli_fetch_array($search_res)) {
                ...
                echo "<td>" . $count . "</td>";
                echo "<td>" . $row['name'] . "</td>";
                echo "<td>" . $row['purpose_dispatch'] . "</td>";
                echo "<td>" . $row['dispatch_period'] . "</td>";
