...
if (isset($_POST['submit'])) {
    ...
    $name = $_POST['name'];
    $query = "SELECT employee.*, missions.* FROM employee,missions WHERE employee.id = missions.e_id and missions.c_id = '" . $_SESSION['c_id'] . "' and employee.name like '%" . $name . "%'";
    $search_res = filterTable($query);
} else {
    $query = "SELECT employee.*, missions.* FROM employee,missions WHERE employee.id = missions.e_id and missions.c_id = '" . $_SESSION['c_id'] . "' ";
    $search_res = filterTable($query);
}
    ...
    <div class="searchs">
        ...
        <table class="tab" width=100% border=1px solid black>
            ...
            while ($row = mysqli_fetch_array($search_res)) {
                ...
                echo "<td>" . $count . "</td>";
                echo "<td>" . $row['name'] . "</td>";
                echo "<td>" . $row['ministerial_order'] . "</td>";
                echo "<td>" . $row['university_matter'] . "</td>";
