...
if (isset($_POST['submit'])) {
    ...
    $name = $_POST['name'];
    $query = "SELECT employee.*, training.* FROM employee,training WHERE employee.id = training.e_id and training.c_id = '" . $_SESSION['c_id'] . "' and employee.name like '%" . $name . "%'";
    $search_res = filterTable($query);
} else {
    $query = "SELECT employee.*, training.* FROM employee,training WHERE employee.id = training.e_id and training.c_id = '" . $_SESSION['c_id'] . "' ";
    $search_res = filterTable($query);
}
