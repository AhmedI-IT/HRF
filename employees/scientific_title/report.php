...
if (isset($_POST['submit'])) {
    ...
    $name = $_POST['name'];
    $query = "SELECT employee.*, scientific_title.* FROM employee,scientific_title WHERE employee.id = scientific_title.e_id and scientific_title.c_id = '" . $_SESSION['c_id'] . "' and employee.name like '%" . $name . "%'";
    $search_res = filterTable($query);
} else {
    $query = "SELECT employee.*, scientific_title.* FROM employee,scientific_title WHERE employee.id = scientific_title.e_id and scientific_title.c_id = '" . $_SESSION['c_id'] . "' ";
    $search_res = filterTable($query);
}
...
<head>
    ...
    <title>تقارير اللقب العلمي</title>
</head>
