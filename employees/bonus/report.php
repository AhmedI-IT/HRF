...
if (isset($_POST['submit'])) {
    ...
    $name = $_POST['name'];
    $query = "SELECT employee.*, bonus.* FROM employee,bonus WHERE employee.id = bonus.e_id and bonus.c_id = '" . $_SESSION['c_id'] . "' and employee.name like '%" . $name . "%'";
    $search_res = filterTable($query);
} else {
    $query = "SELECT employee.*, bonus.* FROM employee,bonus WHERE employee.id = bonus.e_id and bonus.c_id = '" . $_SESSION['c_id'] . "' ";
    $search_res = filterTable($query);
}
