...
if (isset($_POST['submit'])) {
    ...
    $name = $_POST['name'];
    $query = "SELECT employee.*, vacations.* FROM employee,vacations WHERE employee.id = vacations.e_id and vacations.c_id = '" . $_SESSION['c_id'] . "' and employee.name like '%" . $name . "%'";
    $search_res = filterTable($query);
} else {
    $query = "SELECT employee.*, vacations.* FROM employee,vacations WHERE employee.id = vacations.e_id and vacations.c_id = '" . $_SESSION['c_id'] . "' ";
    $search_res = filterTable($query);
}
    ...
    <div class="searchs">
        ...
        <table class="tab" width=100% border=1px solid black>
            ...
            while ($row = mysqli_fetch_array($search_res)) {
                ...
                echo "<td>" . $count . "</td>";
                echo "<td>" . $row['name'] . "</td>";
                echo "<td>" . $row['kind_hol'] . "</td>";
                echo "<td>" . $row['reason_hol'] . "</td>";
                echo "<td>" . $row['dat'] . "</td>";
