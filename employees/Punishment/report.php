...
if (isset($_POST['submit'])) {
    ...
    $name = $_POST['name'];
    $query = "SELECT employee.*, punishment.* FROM employee,punishment WHERE employee.id = punishment.e_id and punishment.c_id = '" . $_SESSION['c_id'] . "' and employee.name like '%" . $name . "%'";
    $search_res = filterTable($query);
} else {
    $query = "SELECT employee.*, punishment.* FROM employee,punishment WHERE employee.id = punishment.e_id and punishment.c_id = '" . $_SESSION['c_id'] . "' ";
    $search_res = filterTable($query);
}
    ...
    <div class="searchs">
        ...
        <table class="tab" width=100% border=1px solid black>
            ...
            while ($row = mysqli_fetch_array($search_res)) {
                ...
                echo "<td>" . $count . "</td>";
                echo "<td>" . $row['name'] . "</td>";
                echo "<td>" . $row['type_punishment'] . "</td>";
                echo "<td>" . $row['reason_punishment'] . "</td>";
